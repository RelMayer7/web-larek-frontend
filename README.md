...
Все классы представленияотвечают за отображение внутри контейнеров (DOM-элемент) передаваемых в них данных.
...
Отвечают за реализацию форм оплаты и контактной информации, расширяют класс Form.
...
### Класс Page
Отвечает за отображение блока с карточками на главной странице а также кнопки корзины и счетчика товаров в корзине.
...
### Класс LarekApi
Расширяет класс Api. Реализует методы загрузки массива карточек и отправки заказа на сервер. Конструктор принимает CDN_URL, API_URL серевра и параметры запроса.
